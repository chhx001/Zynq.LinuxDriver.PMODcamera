...
#define VIVI_MODULE_NAME "pmod_cam"
...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------*/
/*********************VDMA**************************************/
static void *buffer_base[2];
static void *vdma_base;
#define MAX_FRAMES	5
...
/***************************XVDMA****************************/
extern void xvdma_get_dev_info(u32 device_id, struct xvdma_dev *dev);
extern void xvdma_prep_slave_sg(struct xvdma_buf_info *buf_info);
extern void xvdma_device_control(struct xvdma_chan_cfg *chan_cfg);
extern void dma_async_issue_pending(struct dma_chan *chan);
extern void xvdma_start_transfer(struct xvdma_transfer *tx_info);
...
void startVDMA()
{
	...
	/*fd_vdma = filp_open("/dev/xvdma", O_RDWR,0);
	if(IS_ERR(fd_vdma))
		printk("open error\n");
	printk("fdvdma:%p\n",fd_vdma);	
	if(fd_vdma->f_op->unlocked_ioctl(fd_vdma, XVDMA_GET_NUM_DEVICES, &devices) < 0)
		printk("cant get device nums \n");
	printk("devices:%d\n",devices);
	
	
	
	if(fd_vdma->f_op->unlocked_ioctl(fd_vdma, XVDMA_GET_DEV_INFO, &xvdma_dev) < 0)
		printk("cant get device info\n");*/

	xvdma_get_dev_info(0, &xvdma_dev);
	printk("VDMA device info GET!\n");
	//rx
	...
	chan_cfg.config.vsize = 480;
	chan_cfg.config.hsize = 640 * 3; // width length in bytes (Assuming always a pixel is of 32bpp / 24bpp unpacked)
	chan_cfg.config.stride = 640 * 3;		// stride length in bytes (Assuming always a pixel is of 32bpp / 24bpp unpacked)

	chan_cfg.config.frm_cnt_en = 0; 
	chan_cfg.config.frm_dly = 0;
	chan_cfg.config.park = 1;
	chan_cfg.config.park_frm = 0;
	chan_cfg.config.gen_lock = 0; 
	...
	printk("VDMA config OK!\n");
	//fd_vdma->f_op->unlocked_ioctl(fd_vdma, XVDMA_DEVICE_CONTROL, &chan_cfg);			// --1--
	...
	buf_info.addr_base = 0x19950000;
	buf_info.buf_size = 0x00870000;
	buf_info.frm_cnt = 3;

	xvdma_prep_slave_sg(&buf_info);
	printk("VDMA Buffer OK!\n");
	//fd_vdma->f_op->unlocked_ioctl(fd_vdma, XVDMA_PREP_BUF, &buf_info);

	xfer_param.wait = 0;
	//fd_vdma->f_op->unlocked_ioctl(fd_vdma, XVDMA_GET_DEV_INFO, &xvdma_dev);
	xfer_param.chan = xvdma_dev.rx_chan;
	unsigned int tmp = *(unsigned int *)buffer_base[0];

	xvdma_start_transfer(&xfer_param);
	printk("VDMA start!\n");
	//fd_vdma->f_op->unlocked_ioctl(fd_vdma, XVDMA_START_TRANSFER, &xfer_param);
	
	//filp_close(fd_vdma,0);
	//while(*(unsigned int*)buffer_base[0] == tmp);
	//printk("buffer change success");
	
	iowrite32(0x100,vdma_base+0x28);
...
#define BUFFER_OFFSET 0x00870000
static void vivi_fillbuff(struct vivi_dev *dev, struct vivi_buffer *buf)
{
	...
	//unsigned int vdma_base = phys_to_virt(0x43000000);
	if (!vbuf)
		return;
...
/*************************user*****************************/
void setVDMABuf(struct vivi_dev *dev)
{
	unsigned int tmp = *(unsigned int *)buffer_base[0];
	iowrite32(0x100c1,vdma_base + 0x30); //control,vdma start
	iowrite32(0x19950000,vdma_base + 0xac); //frame buffer 1
	iowrite32(0x19950000 + BUFFER_OFFSET,vdma_base + 0xb0); //frame buffer 2
	iowrite32(0x19950000 + BUFFER_OFFSET * 2,vdma_base + 0xb4); //frame buffer 2
	iowrite32(0x500,vdma_base + 0xa8); //stride
	iowrite32(0x500,vdma_base + 0xa4); //hsize
	iowrite32(0x1e0,vdma_base + 0xa0); //vsize
	//iowrite32(0x0,vdma_base + 0x44); //register index
	//iowrite32(0x0,vdma_base + 0x28); //park
